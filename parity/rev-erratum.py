"""erratum.py — an exact-arithmetic exhibit for the r₁₀ sign erratum.

Purpose: a self-contained, reproducible demonstration suitable for
correspondence with the authors of Habera–Zilian 2025 (arXiv:2511.00292).
In exact rational arithmetic — no floats, no tolerances — this shows that
...
The corrected sign also agrees with the validated x₁₀ term of the 2021
paper (arXiv:2111.02117, Eq. 29), which is part of the evidence.
...
Exit code is nonzero only if the CORRECTED form ever fails — the printed
form's failures are the exhibit, not a gate.
"""
...
def main() -> int:
    ...
    print(f"  Algorithm 8 as printed  = {delta_frac(REGRESSION, -1)}"
          "   (r₁₀ with − q·r·d₂)")
    print(f"  Algorithm 8 corrected   = {delta_frac(REGRESSION, +1)}"
          "   (r₁₀ with + q·r·d₂)\n")

    rng = np.random.default_rng(seed)
    cases = [tuple(Fraction(x) for x in A)
             for A in gen_cases.integer_matrices(rng, n)]
    fail_printed = sum(1 for A in cases
                       if delta_frac(A, -1) != identity_value(A))
    fail_corrected = sum(1 for A in cases
                         if delta_frac(A, +1) != identity_value(A))
    print(f"sweep over {total} random integer matrices (seed {seed}):"
          .format(total=len(cases)))
