...
2. **Deterministic Closed-Form Speed**: Algorithms must be non-iterative, branch-minimized, and execute in sub-microsecond time.
...
- **Strict IEEE 754 Compliance**: All implementations must assume standard 64-bit double precision behavior.
  - Primitive operations ($+$, $-$, $*$, $/$, $\sqrt{}$) are correctly rounded to $\le 0.5$ ULP.
  - Transcendental functions (e.g., `acos`, `cos`) follow host platform accuracy, typically $\le 1.0$ ULP.
  - Numerical gates in tests must be derived from these bounds (see `tests/Tests/Util.lean`).
...
- **NEVER loosen a tolerance gate or modify expected test outputs to make a failing test pass.** If a gate fails, the mathematical algorithm or implementation has regressed, and must be remedied.
- **NEVER edit generated files directly**: The file `generated/golden.json` is generated via 50-digit `mpmath` arithmetic. Update it only using `just golden`.
   - **Note**: a *strong* PR explanation is required if `golden.json` or its generator is modified (see [Golden Test Vectors](#golden-test-vectors)).
...
- Matrix build on `ubuntu-latest` with `leanprover/lean4` across:
   - `v4.27.0` (minimum supported)
   - `v4.30.0`
   - `v4.34.0-rc2` (pinned development toolchain)
...
`linter.missingDocs = true` is enabled, and all public types, structures, fields, and definitions in `src/` **must** have docstrings (`/-- ... -/`) to be properly generated by `doc-gen4`. We follow Lean's [documentation convention](https://github.com/leanprover/lean4/blob/master/doc/style.md) with [Chicago](https://www.chicagomanualofstyle.org/tools_citationguide.html) style citations.
...
*Thank you for helping make scientific computing in Lean 4 better!*
